...
class MujocoEnv(Env):
    def __init__(
        ...
        self.kps = (
            np.array([stiffness[joint.split("_", 1)[1]] for joint in self.robot.all_joints()]) * self.cfg.gains.kp_scale
        )
        self.kds = (
            np.array([damping[joint.split("_", 1)[1]] for joint in self.robot.all_joints()]) * self.cfg.gains.kd_scale
        )
        self.tau_limits = (
            np.array([effort[joint.split("_", 1)[1]] for joint in self.robot.all_joints()]) * self.cfg.gains.tau_factor
        )

        self.default_joint_pos = np.array(
            [self.robot.default_standing().get(joint, 0.0) for joint in self.robot.all_joints()]
        )
    ...
    def reset(self) -> Dict:
        ...
        try:
            ...
            self.data.qpos[-self.num_joints :] = self.default_joint_pos + np.random.uniform(
                -self.cfg.domain_rand.start_pos_noise, self.cfg.domain_rand.start_pos_noise, size=self.num_joints
            )
        except:
