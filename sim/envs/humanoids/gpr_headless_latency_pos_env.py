...
from sim.resources.gpr_headless_latency_pos.joints import Robot
...
class GprHeadlessLatencyPosEnv(LeggedRobotLatency):
    """GprHeadlessLatencyPosEnv is a class that represents a custom environment for a legged robot.
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[(2 * num_joints + 5) : (3 * num_joints + 5)] = 0.0  # previous actions
        noise_vec[(3 * num_joints + 5) : (3 * num_joints + 5) + 3] = (
            noise_scales.quat * self.obs_scales.quat
        )  # projected_gravity
        # noise_vec[(4 * num_joints + 5) + 3 : (4 * num_joints + 5) + 6] = noise_scales.ang_vel * self.obs_scales.ang_vel # ang vel
        return noise_vec
