...
from sim.resources.gpr_headless_pos.joints import Robot

NUM_JOINTS = len(Robot.all_joints())

class GprHeadlessPosCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        frame_stack = 15
        c_frame_stack = 3
        num_single_obs = 8 + NUM_JOINTS * 3 # 1 for past actions, 1 for dof pos, 1 for dof vel
        num_observations = int(frame_stack * num_single_obs)
        single_num_privileged_obs = 25 + NUM_JOINTS * 4 # 1 for past actions, 1 for dof pos, 1 for dof vel, 1 for diff
...
class GprHeadlessPosStandingCfg(GprHeadlessPosCfg):
...
class GprHeadlessPosCfgPPO(LeggedRobotCfgPPO):
    seed = 5
    ...
    class runner:
        ...
        experiment_name = "gpr_headless_pos"
        run_name = ""
        # load and resume
        resume = False
